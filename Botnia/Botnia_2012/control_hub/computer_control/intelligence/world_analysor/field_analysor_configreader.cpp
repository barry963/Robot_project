...
bool ConfigFile::ReadFile(const char * fname)
{
...
bool ConfigReader::ReadFile(char *_fname)
{
    QString filepath=application_path;
    filepath+="/config/";
    filepath+=_fname;
    filepath+=".cfg";

    QByteArray temp_byte = filepath.toLatin1();
    const char* pfilepath= temp_byte.data();
    int reader_i = FindFile(_fname);

    if (reader_i >= 0) {
        if (!filereaders[reader_i].ReadFile(pfilepath)) {
            qDebug() << "ERROR: Cannot reread config file:"<<filepath;
            return false;
        }
...
DataMap *ConfigReader::FindDataMapping(char *_fname, char *_dataname, int _type)
{
    // Make sure the file has been read.
    if (FindFile(_fname) < 0)
    {
        ReadFile(_fname);
    }
...
void ConfigReader::Print(FILE *f)
{
    ...
    for (uint i = 0; i < datamap.size(); i++) {
        ...
        case CR_DOUBLE: {
            ...
            for (uint j = 0; j < datamap[i]->veccount; j++)
                fprintf(f, "%f, ", dptr[j]);
        } break;
        case CR_INT: {

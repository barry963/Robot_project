...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
	cte = 0
        if self.x <= radius and self.x >= -0.3*radius and self.y <= 2.3*radius and self.y >= -0.3*radius:
            cte = sqrt((self.x-radius)**2 + (self.y-radius)**2) - radius
        elif self.x >= radius and self.x <= 3*radius and self.y<=2.3*radius  and self.y>=radius:
            cte = sqrt((self.y-radius)**2)-radius
        elif self.x >= 3*radius and self.x <= 4.3*radius and self.y<=2.3*radius and self.y >= -0.3* radius:
            cte = sqrt((self.x-3*radius)**2 + (self.y-radius)**2)-radius
        elif self.x >= radius and self.x <= 3*radius and self.y<=radius and self.y >= -0.3*radius :
            cte =sqrt((self.y-radius)**2)-radius
	else:
	    print self.x ,self.y
        # 

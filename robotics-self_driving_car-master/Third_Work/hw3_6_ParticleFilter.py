...
distance_noise = 0.0 # Noise parameter: should be included in move function.
...
class robot:
    ...
    def move(self, motion):
        ...
        if abs(alpha) < 0.0001:
            result.set(self.x + distance * cos(self.orientation),
            ...
            result.set(cx + radius * sin(new_orientation),
                       cy - radius * cos(new_orientation),
                       new_orientation)
        return result
    ...
    def sense(self,flag = 1): #do not change the name of this function
        ...
        for landmark  in landmarks:
            Z.append(atan2(self.y-landmark[0],self.x-landmark[1])-self.orientation+pi+random.gauss(.0,self.bearing_noise)*flag)
        return Z
    # copy your code from the previous exercise
